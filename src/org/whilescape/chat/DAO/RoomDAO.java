...
public class RoomDAO {
	...
	// 생성된 채팅방의 정보를 DB에 입력하는 메서드
	public static boolean chat_insert(RoomVO rvo) {
		
		String privateChoice = rvo.getPrivateChoice();
		...
		if(flag) {
			try {
				...
				pstmt.setInt(3 , 0); // 1말고 0으로 해야한다. 그 이유는	roomJoin 메서드에서 방장이 이중 카운트 되기 때문 //nowmem int
	...
	// 방 번호를 param으로 넣어 해당 방의 rvo를 리턴하는 메서드
...
//			e.printStackTrace();
	...
	// 디비에 있는 모든 채팅방 각각의 RVO를 담은 리스트를 반환하는 메서드
	...
	// rvo를 param으로 넣어 방 번호를 반환하는 메서드
